{{-- Recursive folder tree rows for FolderResource ListFolders --}}
@foreach ($nodes as $node)
            ...
            <div class="fi-ta-col">
                <div class="fi-ta-text" style="padding-left: {{ 0.75 + ($depth * 1.25) }}rem">
                    <div class="flex min-w-0 items-center gap-2">
                        ...
                        <a
                            href="{{ \App\Filament\Resources\Folders\FolderResource::getUrl('view', ['record' => $node['id']]) }}"
                            class="truncate text-sm leading-6 font-medium text-primary-600 hover:underline dark:text-primary-400"
                            @mousedown.stop
        ...
        <td class="fi-ta-cell whitespace-nowrap" role="gridcell">
            <div class="fi-ta-actions justify-end gap-4 pe-2 sm:pe-3" @mousedown.stop @dragstart.stop.prevent>
                @if ($this->canManageFolders())
                    <a
                        href="{{ \App\Filament\Resources\Folders\FolderResource::getUrl('edit', ['record' => $node['id']]) }}"
    ...
    @if (! empty($node['children']))
        @include('filament.resources.folders.partials.folder-nodes', [
            'nodes' => $node['children'],
            'depth' => $depth + 1,
        ])
    @endif
