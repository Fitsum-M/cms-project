    ...
    @else
        <div class="grid gap-6 sm:grid-cols-2">
            @foreach ($posts as $post)
                <article class="flex flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition hover:border-blue-200 hover:shadow-md">
                    @if ($post->featuredImageUrl())
                        <a href="{{ route('frontend.posts.show', $post->slug) }}" class="block aspect-video w-full overflow-hidden bg-slate-100">
                            <img src="{{ $post->featuredImageUrl('medium') ?? $post->featuredImageUrl() }}"
                    ...
                    <div class="flex flex-1 flex-col p-6">
                        ...
                        @if ($post->resolvedExcerpt())
                            <p class="mt-3 text-slate-600 line-clamp-3">{{ $post->resolvedExcerpt() }}</p>
                        @endif
                        ...
                        <div class="mt-auto pt-5">
                            <a href="{{ route('frontend.posts.show', $post->slug) }}" class="inline-flex items-center text-sm font-semibold text-blue-600 hover:text-blue-700">
                                Read more
